...
def predict(image_path, model, topk, gpu_usage):
    ...
    ps = torch.exp(logps)
    top_probes, top_classes = ps.topk(topk, dim=1)

    return top_probes, top_classes, image_show
    # TODO: Implement the code to predict the class from an image file

def classifier(image_path, checkpoint, category_names, top_k, gpu_usage): 
...
def main():
    ...
    ################
    classifier(in_arg.test_image_path, in_arg.checkpoint, in_arg.top_k, in_arg.category_names, in_arg.gpu_usage)
    ################
